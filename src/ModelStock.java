...
public class ModelStock {
  ...
  private void movingAverage(Scanner scanner) {
    System.out.println("Type the stock symbol. E.G, Google would be GOOG:");
    String stockSymbol = scanner.nextLine();
    System.out.println("Type the start date in the format: YYYY-MM-DD");
    String startDate = scanner.nextLine();
    System.out.println("Type the number of days that we are looking at, E.G, "
            + "3 for a 3 day moving average");
    int xDays = scanner.nextInt();
    scanner.nextLine();
    URL url = null;
    try {
      /*
    ...
    for (int i = 0; i < outputInLines.length; i++) {
      ...
      if (line.substring(0, 10).equals(startDate)) {
        String[] sections = line.split(",");
        Double average = (Double.parseDouble(sections[4]) - Double.parseDouble(sections[1])) / 2;
